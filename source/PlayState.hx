...
class PlayState extends MusicBeatState
{
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'senpai' | 'roses' | 'thorns':
					...
					schoolIntro(doof);
					case 'golden-ballad':
						startCutscene(cutsceneJson, 'noon');
					case 'silver-symphony':
						startCutscene(cutsceneJson, 'breakdown');
					case 'platinum-vinyl':
						//startVideo('palladium');
						startCutscene(cutsceneJson, 'smth');
	...
	public function finishSong(?ignoreNoteOffset:Bool = false):Void
		{
			/*if(OpenFlAssets.exists(Paths.json(Paths.formatToSongPath(SONG.song) + '/endCutscene')))
				if(isStoryMode)
				{
					//cutsceneDialogue.finishThing = endSong;
					startCutscene(cutsceneJsonCont);
				}*/
	...
	public function endSong():Void
	{
		...
		endingSong = true;
		camZooming = false;
		//if(!seenCutscene) seenCutscene = false;	
		updateTime = false;
		inCutscene = false;
		deathCounter = 0;
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				if(OpenFlAssets.exists(Paths.json(Paths.formatToSongPath(SONG.song) + '/endCutscene')) && !seenCutscene)
					{
						trace('test');
						startCutscene(cutsceneJsonCont);
						seenCutscene = true;
				...
				trace('endSong worked');

				seenCutscene = false;		

				trace('endSong worked 2');
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if(Paths.formatToSongPath(SONG.song) == 'platinum-vinyl')
		{
			var targetsArray:Array<FlxCamera> = [camGame, camHUD];
				for (i in 0...targetsArray.length) {
	...
	function functionFormatter(func:String):Void
		...
		function getCutsceneFiles()
		{
			...
			var file:String = Paths.json(songName + '/endCutscene'); //Checks for json/Grafex cutscene
			if (OpenFlAssets.exists(file)) {
